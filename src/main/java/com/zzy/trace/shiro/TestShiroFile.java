...
public class TestShiroFile {
    public static void main(String[] args) {
        ...
        for (User user : users) {
            if(login(user))
                ...
                System.out.printf("%s \t失败登录，用的密码是 %s\t %n",user.getName(),user.getPassword());
        }
    ...
    private static Subject getSubject() {
        //加载配置文件，并获取工厂
        Factory<SecurityManager> factory = new IniSecurityManagerFactory("classpath:shiro_file.ini");
        //获取安全管理者实例
